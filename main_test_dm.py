...
def main():
    ...
    parser.add_argument('--save_bayer_img', type=bool, default=False, help='Save the mosaiced images')
    ...
    if 'clip' in args.method_name:
        use_clip = True       # clip the intensities into range of [0, 1]
    else:
        use_clip = False
    testsets = 'testset'     # fixed
    results = 'results'       # fixed
    bayer_imgs = 'bayer_images'     # fixed
    result_name = args.testset_name + '_' + args.method_name
    ...
    util.mkdir(E_path)
    B_path = os.path.join(bayer_imgs, args.testset_name)   # B_path, for bayer images
    util.mkdir(E_path)
    util.mkdir(B_path)
...
#            img_L, h_border, w_border = util.image_padding(img_L, 16*4)
        ...
        if args.save_bayer_img:
            util.imsave(img_L, os.path.join(B_path, img_name+ext))
